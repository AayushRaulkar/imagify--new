...
const Header = () => {
  ...
  return (
    <motion.div 
      ...
      initial="hidden"
      animate="visible"
      className="relative flex flex-col justify-center items-center text-center my-10 sm:my-16 px-4"
    >
      ...
      <motion.div 
        ...
        className="hidden xl:flex absolute top-10 -left-12 flex-col gap-2 p-2.5 bg-slate-900/80 backdrop-blur-xl border border-white/15 rounded-2xl shadow-2xl animate-float-slow z-0 cursor-pointer"
        onClick={() => onClickHandler("Cyberpunk owl in neon city")}
      >
        <img src={assets.sample_img_2} alt="AI Artwork" className="w-36 h-36 object-cover rounded-xl shadow-lg" />
      ...
      <motion.div 
        ...
        whileHover={{ scale: 1.08, rotate: -2 }}
        className="hidden xl:flex absolute top-28 -right-12 flex-col gap-2 p-2.5 bg-slate-900/80 backdrop-blur-xl border border-white/15 rounded-2xl shadow-2xl animate-float-medium z-0 cursor-pointer"
        onClick={() => onClickHandler("Surreal landscape digital art")}
      >
        <img src={assets.sample_img_1} alt="AI Artwork" className="w-36 h-36 object-cover rounded-xl shadow-lg" />
        <div className="text-left px-1">
      ...
      {/* Top Pulsing Badges */}
      <motion.div variants={itemVariants} className="flex flex-wrap justify-center items-center gap-3 relative z-10 mb-4">
        <motion.div
          whileHover={{ scale: 1.05 }}
          className="inline-flex items-center gap-2.5 px-5 py-2 rounded-full bg-slate-900/90 border border-purple-500/40 text-purple-200 text-xs sm:text-sm font-semibold shadow-[0_0_25px_rgba(168,85,247,0.35)] backdrop-blur-xl"
        >
          ...
          <motion.img 
            ...
            alt="star" 
            className="w-4 h-4 filter drop-shadow-[0_0_8px_rgba(234,179,8,0.9)]" 
          />
        </motion.div>
      </motion.div>
      ...
      >
        Unleash your imagination powered by Google Gemini generative AI & automatic prompt enhancement. Type any prompt and synthesize masterworks in seconds.
      </motion.p>

      {/* CTA Button with Ambient Orbit Animation */}
      <motion.div variants={itemVariants} className="relative mt-8 z-10">
        <div className="absolute -inset-1 bg-gradient-to-r from-purple-600 via-indigo-600 to-cyan-500 rounded-full blur-xl opacity-70 animate-pulse-glow" />
        <motion.button
          onClick={() => onClickHandler()}
          className="sm:text-lg text-white font-bold glow-gradient px-10 py-4 flex items-center gap-3 rounded-full shadow-[0_0_40px_rgba(124,58,237,0.6)] hover:shadow-[0_0_60px_rgba(124,58,237,0.9)] transition-all duration-300 cursor-pointer relative z-10"
      ...
      >
        ...
        <motion.div whileHover={{ scale: 1.05 }} className="text-center">
          <p className="text-2xl font-black text-indigo-400">Gemini AI</p>
          <p className="text-xs text-slate-300 font-light">Default Primary Engine</p>
        </motion.div>
        <motion.div whileHover={{ scale: 1.05 }} className="text-center">
      ...
      >
        ...
        {samplePrompts.map((prompt, idx) => (
          <motion.button
            ...
            className="text-xs bg-slate-900/80 hover:bg-purple-900/50 border border-white/15 text-slate-200 hover:text-white px-4 py-2 rounded-full transition-all duration-300 cursor-pointer shadow-md backdrop-blur-md"
          >
            "{prompt}"
          </motion.button>
        ))}
        ...
        {Array(6)
          ...
          .map((_, index) => (
            <motion.div
              ...
              onClick={() => onClickHandler(index % 2 === 0 ? "Cyberpunk owl portrait" : "Surreal dreamscape 4k")}
